...
{
    Info<<"KinematicCloud<CloudType>::evolveCloud called here"<<endl;
    if (solution_.coupled())
    {
    ...
    if (solution_.transient())
    {
        ...
        // before it is required. ------ changed here
...
:
    ...
    rhoavg_(rho),
    muavg_(mu),
    alphaavg_(mesh_.objectRegistry::template
                    lookupObject<volScalarField>("alphacavg")),
    g_(g),
    pAmbient_(0.0),
    forces_
    (
...
:
    ...
    surfaceFilmModel_(nullptr),
    UIntegrator_(nullptr),
    sourceFilter_(sourceFilterModel),
    UTrans_
    (
        new volVectorField::Internal
        (
...
template<class CloudType>
void Foam::KinematicCloud<CloudType>::filerSourceTerms()
{
    
    if (sourceFilterModel().getMomentumDiffusionFlag())
    {
        UTrans() = sourceFilterModel().diffusion(UTrans(),"momentum");
        UCoeff() = sourceFilterModel().diffusion(UCoeff(),"momentum");
    }
}


template<class CloudType>
void Foam::KinematicCloud<CloudType>::filerSourceTerms(bool useStep)
{
    if (useStep)
    {
        if (sourceFilterModel().getMomentumDiffusionFlag())
...
template<class CloudType>
Foam::tmp<Foam::volScalarField> Foam::KinematicCloud<CloudType>::thetaFiltered()
{
    tmp<volScalarField> ttheta
    (
        volScalarField::New
        (
            this->name() + ":thetaFiltered",
            mesh_,
    ...
    if (sourceFilterModel().getPhaseDiffusionFlag())
    {
        sourceFilterModel().diffusion(theta,"phase");
    }
...
{
Info<<"KinematicCloud<CloudType>::motion called here"<<endl;    
    td.part() = parcelType::trackingData::tpLinearTrack;
    CloudType::move(cloud, td, solution_.trackTime());
...
template<class CloudType>
labelListList Foam::KinematicCloud<CloudType>::creatStepList
(
    label step,
    scalar b,
    scalar ratio
...
{
    labelListList list(0);
    DynamicList<label> dl(0);
    list.append(dl);
    
    // list size is step, initialize with DynamicLists
    scalarField dpList(step, b);
    forAll(dpList, i)
    {
        dpList[i] = dpList[i]*sqrt((i+1.)/step)/ratio;
        list.append(dl);
    }
    
    // avoid duplicate cell id, if the cell has more than one parcel
    // the smaller particle is treated as the larger one
    tmp<volScalarField> tstepListRecord
    (
        volScalarField::New
        (
            "stepListRecord",
    ...
    forAllConstIter(typename KinematicCloud<CloudType>, *this, iter)
    {
        ...
        if(stepListRecord[celli] < 0)
        {
            ...
            forAll(dpList, i)
            {
                label iiver = step-1-i;
                if( p.d() > dpList[iiver] && stepListRecord[celli] < 0)
                {
                    list[i].append(celli);
                    stepListRecord[celli] = 1;
                }
            }
        }    
    }
    ...
    forAllConstIter(typename KinematicCloud<CloudType>, *this, iter)
    {
        ...
        if(stepListRecord[celli] < 0)//if(p.d() < cellLengthScale_[celli]/2)
        {
            list[step].append(celli);
            stepListRecord[celli] = 1;
        }
    }
