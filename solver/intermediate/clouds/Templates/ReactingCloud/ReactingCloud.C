...
template<class CloudType>
void Foam::ReactingCloud<CloudType>::filerSourceTerms()
{
    ...
    if (this->sourceFilterModel().getMassDiffusionFlag())
    {
        forAll(rhoTrans_, i)
        {
            ...
            forAll(rhoTrans_[i], j)
            {
                mass = max(mass, rhoTrans_[i][j]);
            }

            reduce(mass, maxOp<scalar>());
            
            if (mass > small)
            {
                rhoTrans(i) = this->sourceFilterModel().diffusion(rhoTrans(i),"mass");
            }
        }
    }
}
...
template<class CloudType>
void Foam::ReactingCloud<CloudType>::filerSourceTerms(bool useStep)
{
    if (useStep)
    {
        ...
        if (this->sourceFilterModel().getMassDiffusionFlag())
        {
            ...
            forAll(rhoTrans_, i)
            {
                ...
                forAll(rhoTrans_[i], j)
                {
                    mass = max(mass, mag(rhoTrans_[i][j]));
                }
// Info<<"rhoTrans_ i = "<<i<<endl;
// Info<<"beforeDiffusion"<<endl;
// Info<<rhoTrans_[i]<<endl;
                reduce(mass, maxOp<scalar>());
                
                if (mass > small)
                {
                    rhoTrans(i) = this->sourceFilterModel().diffusion(rhoTrans(i),"mass",list);
                }
// Info<<"afterDiffusion"<<endl;
// Info<<rhoTrans_[i]<<endl;
            }
        }
    }
