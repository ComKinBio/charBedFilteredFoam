...
template<class CloudType>
void Foam::ThermoCloud<CloudType>::filerSourceTerms(bool useStep)
{
    if (useStep)
    {
        ...
        if (this->sourceFilterModel().getHeatDiffusionFlag())
        {
// Info<<" before hsTrans() :"<< hsTrans() << endl;
            hsTrans() = this->sourceFilterModel().diffusion(hsTrans(),"heat", list);
// Info<<" after hsTrans() :"<< hsTrans() << endl;
            if (this->solution().semiImplicit("h"))
            {
                hsCoeff() = this->sourceFilterModel().diffusion(hsCoeff(),"heat", list);
            }
        }
        ...
        if (radiation_)
        {
            if (this->sourceFilterModel().getRadiaDiffusionFlag())
            {
                ...
                radT4() = this->sourceFilterModel().diffusion(radT4(),"radia", list2);
                radAreaPT4() = this->sourceFilterModel().diffusion(radAreaPT4(),"radia", list2);
            }
        }
    }
    else
