...
//MFIX modifed
template<class CloudType>
Foam::scalar Foam::PairSpringSliderDashpotHeat<CloudType>::EVAL_H_PFP
(	scalar RLENS_dim,
	scalar S,
    scalar OLAP_dim,
    scalar RP
...
{
	const scalar RLENS = RLENS_dim/RP;
    const scalar KN = S/RP;
    const scalar OLAP = OLAP_dim/RP;
//     Info<<"EVAL_H_PFP RLENS: "<<RLENS<<endl;
//     Info<<"EVAL_H_PFP KN: "<<KN<<endl;
//     Info<<"EVAL_H_PFP OLAP: "<<OLAP<<endl;
	scalar Rout = sqrt(sqr(RLENS)-sqr(1.0-OLAP));
//     Info<<"EVAL_H_PFP Rout: "<<Rout<<endl;
	Rout=min(Rout,1.0);

	const scalar TERM1 = pi*(sqr(1.0-OLAP)-sqr(1.0-OLAP-KN))/KN;
	const scalar TERM2 = 2.0*pi*(sqrt(1.0-sqr(Rout))-(1.0-OLAP-KN));
    const scalar TERM3 = 2.0*pi*(1.0-OLAP)*log((1.0-OLAP-sqrt(1.0-sqr(Rout)))/KN);
//     Info<<"EVAL_H_PFP TERM1: "<<TERM1<<endl;
//     Info<<"EVAL_H_PFP TERM2: "<<TERM2<<endl;
//     Info<<"EVAL_H_PFP TERM3: "<<TERM3<<endl;
...
) const
{
    ...
    scalar MIN_RAD = min(dAEff/2.,dBEff/2.);

    if (normalOverlapMag > -MIN_RAD)
    {
        // Particles in collision
        ...
        //         ref: http://www.thermalradiation.net/sectionc/C-137.html
//          pA.ppT4sum() += 0.0762*(Foam::pow4(pB.T())-Foam::pow4(pA.T()));
//          pB.ppT4sum() += 0.0762*(Foam::pow4(pA.T())-Foam::pow4(pB.T()));
//        
...
//             
//             
//             // This part is for particle-fluid-particle calcuation modified from MFIX code des_thermo_cond_mod.f
//             //MFIX code 0.2, Rong and Horio 1, Morris et al. 0.4
// 			const scalar FLPC = 0.4;
// 			const scalar MAX_RAD = max(dAEff/2.,dBEff/2.);
// 			const scalar MIN_RAD = min(dAEff/2.,dBEff/2.);
//             //MFIX code 4.0e-10, Rong and Horio 4.0e-10, Morris et al. 2.75e-8
// 			const scalar DES_MIN_COND_DIST = 2.75e-8;
// 			//Heron's formula to calculate r_l(not the same method in MFIX to calculate RD_OUT)
// 			//ref Fig1 of Progress in Energy and Combustion Science 79 (2020) 100847
// //             Info<<"2222222222222"<<endl;
// 			const scalar HF_s_1 = (r_AB_mag + MAX_RAD*(1.0+FLPC) + MIN_RAD)/2.;
// 			const scalar HF_A_1 = sqrt(mag(HF_s_1*(HF_s_1 - r_AB_mag)*(HF_s_1 - MAX_RAD*(1.0+FLPC))*(HF_s_1 - MIN_RAD)));
// 			const scalar r_l = 2.0*HF_A_1/r_AB_mag;
// 			//MFIX get overlap between each particle and contact plane
// 			scalar RATIO = normalOverlapMag / MAX_RAD;
// 			const scalar OLAP_1 = MAX_RAD*MAX_RAD*(RATIO-0.5*RATIO*RATIO)/r_AB_mag;
// 			const scalar OLAP_2 = normalOverlapMag - OLAP_1;
// //             Info<<"33333333"<<endl;
// 			//MFIX get effective minimum conduction distance for each particle
// 			RATIO = (normalOverlapMag + DES_MIN_COND_DIST) / MAX_RAD;
//             const scalar S_1 = (MAX_RAD*MAX_RAD*(RATIO-0.5*RATIO*RATIO)/(r_AB_mag-DES_MIN_COND_DIST)) - OLAP_1;
//             const scalar S_2 = DES_MIN_COND_DIST - S_1;
// 			//MFIX get effective lens radius for particle-contact plane
//             const scalar RLENS_1 = sqrt(r_l*r_l+sqr(MIN_RAD-OLAP_1));
//             const scalar RLENS_2 = sqrt(r_l*r_l+sqr(MAX_RAD-OLAP_2));
// //             Info<<"MAX_RAD: "<<MAX_RAD<<endl;
// //             Info<<"MIN_RAD: "<<MIN_RAD<<endl;
// //             Info<<"r_l: "<<r_l<<endl;
// //             Info<<"OLAP_1: "<<OLAP_1<<endl;
// //             Info<<"OLAP_2: "<<OLAP_2<<endl;
// //             Info<<"S_1: "<<S_1<<endl;
// //             Info<<"S_2: "<<S_2<<endl;
// //             Info<<"RLENS_1: "<<RLENS_1<<endl;
// //             Info<<"RLENS_2: "<<RLENS_2<<endl;
// 
// 			//conductivity of gas W m-1 K-1
// 			const scalar k_gas = 0.025;
// 
// 			const scalar H_1 = EVAL_H_PFP(RLENS_1, S_1, OLAP_1,MIN_RAD)*MIN_RAD*k_gas;
//             const scalar H_2 = EVAL_H_PFP(RLENS_2, S_2, OLAP_2,MAX_RAD)*MAX_RAD*k_gas;
// //             Info<<"H_1: "<<H_1<<endl;
// //             Info<<"H_2: "<<H_2<<endl;
// 			const scalar H = H_1*H_2/(H_1+H_2);
// 
// 			Q_PtoFtoP = H * dT_AB;
// 
