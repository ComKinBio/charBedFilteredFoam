...
tmp<volScalarField>
Foam::gasFilter::filteredField(const volScalarField& F, labelListList list)
{
    if (implicitFvm_.value())
    {
        ...
        label stepNumber = list.size();
        
        while (simple_.loop(diffusionRunTime_))
        {
            label stepi =  stepNumber - diffusionRunTime_.timeIndex();
            ...
            if (diffusionRunTime_.timeIndex() == 1)
            {
                while (simple_.correctNonOrthogonal())
                {
                    ...
                    {
                        iS[list[stepi][i]]
                      = diffWorkFieldInterFeildRef[list[stepi][i]];
                    }
        ...
        {
            iS[list[0][i]]
            = diffWorkFieldInterFeildRef[list[0][i]];
        }
    ...
    else
    {
        FatalErrorInFunction
            << "Explicit method for gas filtering is not availble "<< nl
            << exit(FatalError);
        return F;
    }
}
...
tmp<volVectorField>
Foam::gasFilter::filteredField(const volVectorField& F, labelListList list)
{
    if (implicitFvm_.value())
    {
